#!/usr/bin/env python3
"""Generate Better Companions' mod menu icon: a cow, and a heart over it.

The cow's own face out of its entity sheet, and the hearts a tamed animal
shows, which is the whole mod in one look: the animals that could never be
befriended, befriended. Source pixels are read straight out of the vanilla
...
OUT = os.path.join(HERE, "src/main/resources/assets/better-companions-justfatlard/icon.png")
...
FACE = (6, 6, 8, 8)      # the head's front face on the 64x64 cow sheet
HORN = (22, 0, 1, 3)     # one horn, as the sheet draws it


def build_icon():
    cow = vanilla("entity/cow/cow_temperate.png")
    sprite = blank()
    face = crop(cow, *FACE)
    stamp(sprite, face, 4, 8)
    horn = crop(cow, *HORN)
    stamp(sprite, horn, 3, 7)
    stamp(sprite, horn, 12, 7)
    heart = vanilla("gui/sprites/hud/heart/full.png")
    stamp(sprite, heart, 4, -1)
